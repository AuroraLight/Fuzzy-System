...
class RBFN:
    ...
    def train(self):
        ...
        for i in range(self.iteration):  # 執行設定的迭代次數，期望得到的E越好越好
            ...
            #pso_update_start_time = time.clock()
            #update_min_vector_start_time = time.clock()
            self.pso.update_min_vector(self.E, self.individual_vector)
            #update_min_vector_end_time = time.clock()
            #print("update_min_vector execution time:", update_min_vector_end_time - update_min_vector_start_time)
            '''
            print("update pso.global_min_e:", self.pso.global_min_e)
            print("update pso.global_min_vector:", self.pso.global_min_vector)
            print("update pso.individual_min_e:", self.pso.individual_min_e)
            print("update pso.individual_min_vector:", self.pso.individual_min_vector)
            '''
            #print("individual_vector:", self.individual_vector)

            #update_vector_start_time = time.clock()
            self.pso.update_individual_vector(self.individual_vector)
            #update_vector_end_time = time.clock()
            #print("update_vector execution time:", update_vector_end_time - update_vector_start_time)
            #print("update individual_vector:", self.individual_vector)

            #pso_update_end_time = time.clock()
            #print("pso_update execution time:", pso_update_end_time - pso_update_start_time)
        ...
        #print("pso.global_min_vector:", self.pso.global_min_vector)
        return self.pso.global_min_vector
    ...
    def __init__(self, rbfn_parameter, data):
        ...
        self.selfWeight = float(rbfn_parameter[2])
        self.socialWeight = float(rbfn_parameter[3])
        ...
        # initial pso
        self.pso = PSO.PSO(self.groupNumber, self.selfWeight, self.socialWeight,
